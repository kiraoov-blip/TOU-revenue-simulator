...
function buildRates(versionId){const rows=categoryTariffs().filter(r=>text(r["버전ID"])===versionId&&text(r["계약전력구간"])===$("contract").value&&text(r["전압구분"])===$("voltage").value&&text(r["선택요금"])===$("choice").value),rates={};SEASONS.forEach(s=>rates[s]={"경부하":0,"중간부하":0,"최대부하":0});rows.forEach(r=>{const s=text(r["계절"]),p=text(r["부하시간대"]);if(rates[s]&&PERIODS.includes(p))rates[s][p]=number(r["전력량요금"])});return rates}
...
function updateWarning(){const proxy=text(state.selectedCatalog["대리요금적용"]).toUpperCase()==="Y",dates=state.usage.map(r=>r.date).sort((a,b)=>a-b),period=dates.length?`${ymd(dates[0])}~${ymd(dates.at(-1))}`:"자료 없음";$("warning").innerHTML=proxy?`<b>환산 분석:</b> ${text(state.selectedCatalog["표시명"])} 부하에 ${text(state.selectedCatalog["요금그룹ID"])} 요금표를 대리 적용함. 사용량 자료기간 ${period}`:`<b>데이터 범위:</b> ${text(state.selectedCatalog["표시명"])} · ${period} · ${state.usage.length.toLocaleString()}일`}
...
function calculate(){if(!state.scenarioSchedule||!state.scenarioRates)return;const bb=buildSchedule($("baseVersion").value),br=buildRates($("baseVersion").value),base=calc(bb.schedule,br,bb.discount,true),sch=calc(state.scenarioSchedule,br,bb.discount,true),rat=calc(state.scenarioSchedule,state.scenarioRates,bb.discount,true),fin=calc(state.scenarioSchedule,state.scenarioRates,state.scenarioDiscount,$("useScenarioDiscount").checked),delta=fin.totalRev-base.totalRev;$("kUsage").textContent=fmtEnergy(fin.totalUsage);$("kPeriod").textContent=`${ymd(fin.minDate)}~${ymd(fin.maxDate)}`;$("kBase").textContent=fmtWon(base.totalRev);$("kScenario").textContent=fmtWon(fin.totalRev);$("kDelta").textContent=signedWon(delta);$("kDelta").className=cssSign(delta);$("kDeltaPct").textContent=base.totalRev?`${delta>=0?"+":""}${(delta/base.totalRev*100).toFixed(3)}%`:"-";$("kBaseAvg").textContent=base.totalUsage?(base.totalRev/base.totalUsage).toFixed(2):"-";$("kScenarioAvg").textContent=fin.totalUsage?(fin.totalRev/fin.totalUsage).toFixed(2):"-";const ds=sch.totalRev-base.totalRev,dr=rat.totalRev-sch.totalRev,dd=fin.totalRev-rat.totalRev;[["dSchedule",ds],["dRate",dr],["dDiscount",dd]].forEach(([id,v])=>{$(id).textContent=signedWon(v);$(id).className=cssSign(v)});renderSeasonTable(base,fin);renderPeriodTable(base,fin);renderChart(fin);state.lastResult={base,fin,ds,dr,dd}}
...
function renderChart(r){const svg=$("loadChart"),w=900,h=285,l=58,rr=18,t=36,b=35,vals=r.hourUsage.map(v=>r.days?v/r.days:0),max=Math.max(...vals,1)*1.08,x=i=>l+i*(w-l-rr)/23,y=v=>h-b-v/max*(h-t-b),day=$("dayType").value;let out="";for(let i=0;i<24;i++){const p=state.scenarioSchedule[state.activeSeason][day][i],xx=l+i*(w-l-rr)/24,ww=(w-l-rr)/24;out+=`<rect x="${xx}" y="3" width="${ww-1}" height="19" fill="var(--${PERIOD_CLASS[p]})"/><text x="${xx+ww/2}" y="16" text-anchor="middle" font-size="8">${p[0]}</text>`}for(let k=0;k<=4;k++){const yy=t+(h-t-b)*k/4;out+=`<line x1="${l}" y1="${yy}" x2="${w-rr}" y2="${yy}" stroke="#e4e8ed"/><text x="${l-7}" y="${yy+4}" text-anchor="end" font-size="9" fill="#687386">${((max*(4-k)/4)/1e6).toFixed(1)}</text>`}out+=`<text x="8" y="${t-7}" font-size="9" fill="#687386">백만 kWh/일</text><polyline points="${vals.map((v,i)=>`${x(i)},${y(v)}`).join(" ")}" fill="none" stroke="#2468ad" stroke-width="3"/>`;vals.forEach((v,i)=>{out+=`<circle cx="${x(i)}" cy="${y(v)}" r="2.3" fill="#2468ad"/>`;if(i%2===0)out+=`<text x="${x(i)}" y="${h-13}" text-anchor="middle" font-size="9" fill="#687386">${i}</text>`});svg.innerHTML=out}
...
$("reloadData").onclick=loadGithubData;$("localFile").onchange=e=>{const f=e.target.files[0];if(!f)return;const reader=new FileReader();reader.onload=()=>loadWorkbook(reader.result,"PC 임시");reader.readAsArrayBuffer(f);e.target.value=""};$("category").onchange=selectCategory;$("contract").onchange=()=>{populateVoltage();populateVersions();loadScenarioPreset()};$("voltage").onchange=()=>{populateChoice();populateVersions();loadScenarioPreset()};$("choice").onchange=()=>{populateVersions();loadScenarioPreset()};$("baseVersion").onchange=renderAll;$("scenarioVersion").onchange=loadScenarioPreset;$("year").onchange=calculate;$("scope").onchange=calculate;$("dayType").onchange=renderAll;$("useScenarioDiscount").onchange=calculate;$("loadScenario").onclick=loadScenarioPreset;$("copyBase").onclick=copyBase;$("exportCsv").onclick=exportCsv;document.querySelectorAll("#seasonTabs .tab").forEach(btn=>btn.onclick=()=>{state.activeSeason=btn.dataset.season;document.querySelectorAll("#seasonTabs .tab").forEach(x=>x.classList.toggle("active",x===btn));renderAll()});document.querySelectorAll(".rate-adjust").forEach(btn=>btn.onclick=()=>{const m=number(btn.dataset.mult);SEASONS.forEach(s=>PERIODS.forEach(p=>state.scenarioRates[s][p]=Math.round(state.scenarioRates[s][p]*m*10)/10));renderAll()});loadGithubData();